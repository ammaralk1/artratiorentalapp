...
```bash
npm install
npm run backoffice:local:api
npm run dev
```

- الأمر `npm run backoffice:local:api` يشغل خادم PHP المحلي للـ API على `http://127.0.0.1:8000/api`.
...
- في التطوير المحلي، الواجهة الأمامية تستدعي `/backend/api/...` ويقوم Vite بعمل proxy لها إلى `127.0.0.1:8000/api` افتراضيًا.
- اترك `VITE_API_BASE_URL` غير معيّن في التطوير العادي حتى تبقى طلبات المصادقة عبر نفس الأصل `same-origin` من خلال Vite proxy.
- إذا شغّلت الواجهة على `localhost:5173` فلا تخلطها مع API مباشر على `127.0.0.1:8000` داخل المتصفح، لأن هذا قد يسبب مشاكل في ثبات جلسة تسجيل الدخول محليًا.
- إذا أردت تشغيل الواجهة نفسها على `127.0.0.1` بدل `localhost` فاستعمل:
...
### ملاحظات التطوير المحلي

- الراوتر الخلفي يقبل الآن المسارات بالشكلين معًا:
  - `/api/technicians`
  - `/api/technicians/`
- هذا مهم لأن الواجهة الحالية ترسل عدة طلبات تنتهي بـ `/`، وبدون هذا التوافق كانت بعض نقاط الـ API ترجع `404 Not found` محليًا.
- إذا ظهرت لك بقايا سلوك قديم بعد تغيير إعدادات التطوير المحلي، أعد تشغيل Vite وخادم PHP ثم نفّذ تحديثًا قسريًا للمتصفح `Hard Refresh`.
...
- يتم إنشاء نسخة احتياطية لقاعدة البيانات يوميًا تلقائيًا الساعة 02:10 صباحًا (توقيت الخادم).
- يتم رفع النسخ إلى iDrive e2 (S3‑compatible). صفحة الخدمة: https://www.idrive.com/s3-storage-e2/
- النسخ المحلية تحفظ في المسار: `~/backups/mysql/` لمدة 14 يومًا، ثم تُحذف الأقدم تلقائيًا.
- يمكن تغيير توقيت الجدولة أو مدة الاحتفاظ من مهمة الـ Cron في الخادم عند الحاجة.
