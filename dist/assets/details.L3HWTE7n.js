import{d as dt,v as ma,n,y as lt,t as a}from"./auth.EkPXpg4F.js";import{n as G,i as Q,j as Tt}from"./state.DofjCv9v.js";import{a as ya,i as ba,h as ha,z as S,A as C,b as va,f as fa,J as _a}from"./reservationsService.B7Ff-z7Z.js";const $a=new Map([["available","available"],["متاح","available"],["متوفر","available"],["جاهز","available"],["ready","available"],["reserved","reserved"],["محجوز","reserved"],["محجوزة","reserved"],["maintenance","maintenance"],["صيانة","maintenance"],["under_maintenance","maintenance"],["retired","retired"],["متوقف","retired"],["خارج الخدمة","retired"]]),Aa=new Set(["maintenance","reserved","retired"]);function Sa(e){const f=String(e??"").trim().toLowerCase();return f&&$a.get(f)||"available"}function xa(e){return e?typeof e=="object"?e:Na(e):null}function Pa(e){const f=xa(e);return f?Sa(f.status||f.state||f.statusLabel||f.status_label):"available"}function ga(e){return!Aa.has(Pa(e))}function wa(e={}){return e.image||e.imageUrl||e.img||""}function qa(e){if(!e)return null;const f=G(e),{equipment:z=[]}=dt();return(z||[]).find(B=>G(B?.barcode)===f)||null}function Na(e){const f=G(e);if(!f)return null;const{equipment:z=[]}=dt();return(z||[]).find(B=>G(B?.barcode)===f)||null}function $(e=""){return String(e).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function ct(e){if(e==null)return"";const f=String(e).trim();return f?n(f):""}function Ia(e,f,z=[],B,N=null){const{projectLinked:ut,effectiveConfirmed:pt}=ya(e,N),Dt=e.paid===!0||e.paid==="paid",gt=ba(e),O=e.items||[];let{groups:D}=ha(e);const qt=t=>!!(t&&typeof t=="object"&&(t.type==="package"||Array.isArray(t.packageItems)&&t.packageItems.length||Array.isArray(t.items)&&t.items.some(i=>i&&i.type==="package"))),It=t=>{const i=(t?.package_code??t?.packageDisplayCode??t?.barcode??t?.description??(Array.isArray(t?.items)&&t.items[0]?.barcode)??"").toString().trim().toLowerCase();return n(i)},mt=(t,i)=>{const l=y=>{const A=Array.isArray(y?.items)?y.items[0]:null,v=[A?.price,A?.unit_price,A?.unitPrice,y?.unitPrice,y?.totalPrice];for(const q of v){const r=C(q);if(Number.isFinite(r)&&r>0)return r}return 0},m=l(t),h=l(i);return m&&h?m<=h?t:i:m?t:i},g=[],V=new Map;D.forEach(t=>{if(!qt(t)){g.push(t);return}const i=It(t);if(!i){if(!V.has("__unknown__"))V.set("__unknown__",g.length),g.push(t);else{const l=V.get("__unknown__");g[l]=mt(g[l],t)}return}if(!V.has(i))V.set(i,g.length),g.push(t);else{const l=V.get(i);g[l]=mt(g[l],t)}}),D=g;const{technicians:yt=[]}=dt(),Et=[].concat(Array.isArray(z)?z:[]).concat(Array.isArray(yt)?yt:[]),X=new Map;Et.forEach(t=>{if(!t||t.id==null)return;const i=String(t.id),l=X.get(i)||{};X.set(i,{...l,...t})});const K=(Array.isArray(e.crewAssignments)&&e.crewAssignments.length?e.crewAssignments:Array.isArray(e.techniciansDetails)&&e.techniciansDetails.length?e.techniciansDetails:(e.technicians||[]).map(t=>({technicianId:t}))).map((t,i)=>{const l=t?.technicianId!=null?X.get(String(t.technicianId)):null;let m=t.positionLabel??t.position_name??t.position_label??t.role??t.position??"";(!m||m.trim()==="")&&(m=t.positionLabelAr??t.position_label_ar??t.positionLabelEn??t.position_label_en??t.position_name_ar??t.position_name_en??"");const h=t.positionLabelAlt??t.position_label_alt??t.positionLabelEn??t.position_label_en??t.positionLabelAr??t.position_label_ar??"";let y=m,A=h;if(!y||y.trim()==="")try{const r=Q?Q():[];let s=null;if(t.positionId!=null&&(s=r.find(d=>String(d.id)===String(t.positionId))||null),!s){const d=t.positionKey??t.position_key??t.positionName??t.position_name??t.position??"";if(d&&(s=typeof Tt=="function"?Tt(d):null,!s&&r.length)){const b=String(d).trim().toLowerCase();s=r.find(P=>[P.name,P.labelAr,P.labelEn].filter(Boolean).map(u=>String(u).toLowerCase()).includes(b))||null}}s&&(y=s.labelAr||s.labelEn||s.name||"",(!A||String(A).trim()==="")&&(s.labelAr&&s.labelEn?A=y===s.labelAr?s.labelEn:s.labelAr:A=s.labelAr||s.labelEn||""))}catch{}const v=S(C(t.positionCost??t.position_cost??t.cost??t.daily_wage??t.dailyWage??l?.dailyWage??l?.wage??0)),q=S(C(t.positionClientPrice??t.position_client_price??t.client_price??t.clientPrice??t.daily_total??t.dailyTotal??t.total??l?.dailyTotal??l?.total??l?.total_wage??0));return{assignmentId:t.assignmentId??t.assignment_id??`crew-${i}`,positionId:t.positionId??t.position_id??null,positionKey:t.positionKey??t.position_key??t.positionName??t.position_name??t.position??null,positionLabel:y,positionLabelAlt:A,positionLabelAr:t.positionLabelAr??t.position_label_ar??null,positionLabelEn:t.positionLabelEn??t.position_label_en??null,positionCost:v,positionClientPrice:q,technicianId:t.technicianId!=null?String(t.technicianId):l?.id!=null?String(l.id):null,technicianName:t.technicianName??t.technician_name??l?.name??null,technicianRole:t.technicianRole??l?.role??null,technicianPhone:t.technicianPhone??l?.phone??null,notes:t.notes??null}}),Rt=ma(),F=va(e.start,e.end),Ht=e.discount??e.discountValue??e.discount_value??e.discountAmount??0,bt=C(Ht),Mt=Number.isFinite(bt)?bt:0,Bt=e.discountType??e.discount_type??e.discountMode??"percent",jt=String(Bt).toLowerCase()==="amount"?"amount":"percent",ht=!!(e.applyTax??e.apply_tax??e.taxApplied),vt=C(e.cost??e.total??e.finalTotal),ft=Number.isFinite(vt),zt=ft?S(vt):0,_t=e.companySharePercent??e.company_share_percent??e.companyShare??e.company_share,W=_t!=null?C(_t):Number.NaN,Vt=((e.companyShareEnabled??e.company_share_enabled??e.companyShareApplied)===!0||Number.isFinite(W)&&W>0)&&Number.isFinite(W)?W:0,T=fa({items:O,technicianIds:e.technicians||[],crewAssignments:K,discount:Mt,discountType:jt,applyTax:ht,start:e.start,end:e.end,companySharePercent:Vt,groupingSource:e}),Y=S(T.equipmentTotal),$t=S(T.crewTotal),Kt=S(T.crewCostTotal),Z=S(T.discountAmount),Ut=S(T.subtotalAfterDiscount),tt=Number.isFinite(T.companySharePercent)?T.companySharePercent:0;let et=S(T.companyShareAmount);et=tt>0?S(Math.max(0,et)):0;const at=S(T.taxAmount),Qt=S(T.finalTotal),At=ft?zt:Qt,St=S(T.netProfit),Gt=n(String(e.reservationId??e.id??"")),Wt=e.start?n(lt(e.start)):"-",Jt=e.end?n(lt(e.end)):"-",Ot=n(String(K.length)),Xt=n(Y.toFixed(2)),Yt=n(Z.toFixed(2)),Zt=n(Ut.toFixed(2)),te=n(at.toFixed(2)),ee=n((Number.isFinite(At)?At:0).toFixed(2)),ae=n(String(F)),_=a("reservations.create.summary.currency","SR"),ie=a("reservations.details.labels.discount","الخصم"),ne=a("reservations.details.labels.tax","الضريبة (15%)"),se=a("reservations.details.labels.crewTotal","إجمالي الفريق"),oe=a("reservations.details.labels.subtotalAfterDiscount","الإجمالي"),re=a("reservations.details.labels.duration","عدد الأيام"),le=a("reservations.details.labels.companyShare","🏦 نسبة الشركة"),ce=a("reservations.details.labels.netProfit","💵 صافي الربح"),de=a("reservations.create.equipment.imageAlt","صورة"),E={item:a("reservations.equipment.table.item","المعدة"),quantity:a("reservations.equipment.table.quantity","الكمية"),unitPrice:a("reservations.equipment.table.unitPrice","سعر الوحدة"),total:a("reservations.equipment.table.total","الإجمالي"),actions:a("reservations.equipment.table.actions","الإجراءات")},ue=a("reservations.details.noItems","📦 لا توجد معدات ضمن هذا الحجز حالياً."),pe=a("reservations.details.noCrew","😎 لا يوجد فريق مرتبط بهذا الحجز.");a("reservations.details.technicians.roleUnknown","غير محدد");const me=a("reservations.details.technicians.phoneUnknown","غير متوفر");a("reservations.details.technicians.wage","{amount} {currency} / اليوم");const ye=a("reservations.list.status.confirmed","✅ مؤكد"),be=a("reservations.list.status.pending","⏳ غير مؤكد"),he=a("reservations.list.payment.paid","💳 مدفوع"),ve=a("reservations.list.payment.unpaid","💳 غير مدفوع"),fe=a("reservations.list.payment.partial","💳 مدفوع جزئياً"),_e=a("reservations.list.status.completed","📁 منتهي"),$e=a("reservations.details.labels.id","🆔 رقم الحجز"),Ae=a("reservations.details.section.bookingInfo","بيانات الحجز"),Se=a("reservations.details.section.paymentSummary","ملخص الدفع"),xe=a("reservations.details.labels.finalTotal","المجموع النهائي"),Pe=a("reservations.details.section.crew","😎 الفريق الفني"),we=a("reservations.details.crew.count","{count} عضو"),Ne=a("reservations.details.section.items","📦 المعدات المرتبطة"),Le=a("reservations.details.items.count","{count} عنصر"),ke=a("reservations.details.actions.edit","✏️ تعديل"),Ce=a("reservations.details.actions.delete","🗑️ حذف"),Fe=a("reservations.details.labels.customer","العميل"),Te=a("reservations.details.labels.contact","رقم التواصل"),De=a("reservations.details.labels.project","📁 المشروع المرتبط");a("reservations.details.project.unlinked","غير مرتبط بأي مشروع.");const ge=a("reservations.edit.project.missing","⚠️ المشروع غير متوفر (تم حذفه)"),qe=a("reservations.details.actions.openProject","📁 فتح المشروع"),Ie=a("reservations.details.labels.start","بداية الحجز"),Ee=a("reservations.details.labels.end","نهاية الحجز"),Re=a("reservations.details.labels.notes","ملاحظات"),He=a("reservations.list.noNotes","لا توجد ملاحظات"),Me=a("reservations.details.labels.itemsCount","عدد المعدات"),Be=a("reservations.details.labels.itemsTotal","إجمالي المعدات"),je=a("reservations.paymentHistory.title","سجل الدفعات"),ze=a("reservations.paymentHistory.empty","لا توجد دفعات مسجلة"),Ve=a("reservations.list.unknownCustomer","غير معروف"),it=typeof N?.paymentStatus=="string"?N.paymentStatus.toLowerCase():null,J=ut&&it&&["paid","partial","unpaid"].includes(it)?it:e.paidStatus??e.paid_status??(Dt?"paid":"unpaid"),nt=J==="partial",xt=J==="paid"?he:nt?fe:ve;function Ke(t){if(t==null)return Number.NaN;if(typeof t=="number")return Number.isFinite(t)?t:Number.NaN;const i=String(t).replace(/[^0-9.+-]/g,""),l=Number(i);return Number.isFinite(l)?l:Number.NaN}const st=(t={})=>{const i=String(t.type??t.kind??t.category??"").toLowerCase();return!!(["package","bundle","pack"].includes(i)||Array.isArray(t.packageItems)&&t.packageItems.length)},Ue=(t={})=>[t.packageId,t.package_id,t.packageCode,t.package_code,t.bundleId,t.bundle_id].some(i=>i!=null&&i!==""),Qe=(t={})=>!t||typeof t!="object"?!1:!st(t)&&Ue(t);let ot=Array.isArray(D)&&D.length?D.length:(Array.isArray(O)?O.filter(t=>t&&typeof t=="object"&&!Qe(t)).length:0)||1;ot=Math.max(1,Math.round(ot));const Ge=n(String(ot)),Pt=Le.replace("{count}",Ge),We=we.replace("{count}",Ot),Je=e.notes?n(e.notes):He,Oe=n($t.toFixed(2)),Xe=n(Kt.toFixed(2)),Ye=n(String(tt)),Ze=n(et.toFixed(2)),ta=`${Ye}% (${Ze} ${_})`,ea=Number.isFinite(St)?Math.max(0,St):0,aa=n(ea.toFixed(2)),R=[{icon:"💼",label:Be,value:`${Xt} ${_}`}];R.push({icon:"😎",label:se,value:`${Oe} ${_}`});const ia=a("reservations.details.labels.crewCost","تكلفة الفريق");R.push({icon:"💵",label:ia,value:`${Xe} ${_}`}),Z>0&&R.push({icon:"💸",label:ie,value:`${Yt} ${_}`}),R.push({icon:"📊",label:oe,value:`${Zt} ${_}`}),ht&&at>0&&R.push({icon:"🧾",label:ne,value:`${te} ${_}`}),tt>0&&R.push({icon:"🏦",label:le,value:ta}),R.push({icon:"💵",label:ce,value:`${aa} ${_}`}),R.push({icon:"💰",label:xe,value:`${ee} ${_}`});const na=R.map(({icon:t,label:i,value:l})=>`
  ...
  `).join("");let wt="";try{const i=new URL(window.location.href).searchParams.get("debugFinance");if(i==="1"||i==="true"){const l=r=>(r?.type||"").toLowerCase()==="package"&&String(r?.pricingMode||"").toLowerCase()==="fixed"?"fixed":"daily",m=r=>{if((r?.type||"").toLowerCase()==="package")return!1;const s=Array.isArray(r?.items)?r.items:[];if(!s.length)return!1;if(s.some(u=>u?.reservation_id!=null||u?.reservationId!=null))return!0;const b=s.every(u=>u?.unit_price!=null||u?.unitPrice!=null),P=s.some(u=>u?.daily_rate!=null||u?.dailyRate!=null||u?.unit_rate!=null||u?.unitRate!=null||u?.price!=null);return b&&!P};let h=0,y=0,A=0,v=0;const q=(Array.isArray(D)?D:[]).map((r,s)=>{const d=Number.isFinite(Number(r?.quantity))?Number(r.quantity):0,b=Number.isFinite(Number(r?.unitPrice))?Number(r.unitPrice):0,P=l(r);let u=P==="fixed"?d*b:d*b*F,L="";if((r?.type||"").toLowerCase()==="package")try{const w={package_code:r?.package_code||r?.packageDisplayCode||r?.barcode||r?.packageId||r?.key,packageItems:Array.isArray(r?.packageItems)?r.packageItems:void 0},M=Number(r?.unitPrice);let c;if(Number.isFinite(M)&&M>0)c=d*M;else{const o=_a(w,{packageQuantity:d,days:F});c=Number.isFinite(Number(o.perDayTotal))?Number(o.perDayTotal):d*b}u=c*F,h+=c,y+=u;const p=(pricing.lines||[]).map((o,x)=>`
              ...
              </tr>`).join("");L=p||""}catch{}else{const w=m(r),M=w?0:d*b,c=w?d*b:M*F;A+=M,v+=c}return`
          <tr>
            <td>${s+1}</td>
        ...
        </details>`,console.debug("[finance-debug] groups",D,{rentalDays:F,equipmentTotal:Y,crewTotal:$t,discountAmount:Z,taxAmount:at})}}catch{}console.debug("[reservations/details] payment history raw",e.paymentHistory,e.payment_history);let k=[];ut&&N&&(Array.isArray(N.paymentHistory)?k=N.paymentHistory:Array.isArray(N.payment_history)?k=N.payment_history:Array.isArray(N.payments)?k=N.payments:Array.isArray(N.paymentLogs)&&(k=N.paymentLogs)),(!Array.isArray(k)||k.length===0)&&(Array.isArray(e.paymentHistory)?k=e.paymentHistory:Array.isArray(e.payment_history)?k=e.payment_history:Array.isArray(e.paymentLogs)?k=e.paymentLogs:k=[]);const Nt=Array.isArray(k)?k:[],sa=Nt.length?`<ul class="reservation-payment-history-list">${Nt.map(t=>{const i=t?.type==="amount"?a("reservations.paymentHistory.type.amount","دفعة مالية"):t?.type==="percent"?a("reservations.paymentHistory.type.percent","دفعة نسبة"):a("reservations.paymentHistory.type.unknown","دفعة"),l=Number.isFinite(Number(t?.amount))&&Number(t.amount)>0?`${n(Number(t.amount).toFixed(2))} ${_}`:"—",m=Number.isFinite(Number(t?.percentage))&&Number(t.percentage)>0?`${n(Number(t.percentage).toFixed(2))}%`:"—",h=t?.recordedAt?n(lt(t.recordedAt)):"—",y=t?.note?`<div class="payment-history-note">${$(n(t.note))}</div>`:"";return`
          <li>
        ...
        `}).join("")}</ul>`:`<div class="reservation-payment-history-empty">${$(ze)}</div>`,Lt=String(e?.status||e?.reservationStatus||"").toLowerCase(),kt=Lt==="cancelled"||Lt==="canceled",Ct=kt?[{text:a("reservations.list.status.cancelled","❌ ملغي"),className:"status-cancelled"},{text:xt,className:J==="paid"?"status-paid":nt?"status-partial":"status-unpaid"}]:[{text:pt?ye:be,className:pt?"status-confirmed":"status-pending"},{text:xt,className:J==="paid"?"status-paid":nt?"status-partial":"status-unpaid"}];gt&&!kt&&Ct.push({text:_e,className:"status-completed"});const oa=Ct.map(({text:t,className:i})=>`<span class="status-chip ${i}">${t}</span>`).join(""),j=(t,i,l)=>`
    <div class="res-info-row">
      <span class="label">${t} ${i}</span>
      <span class="value">${l}</span>
    </div>
  `;let rt="";if(e.projectId){let t=$(ge);if(N){const i=N.title||a("projects.fallback.untitled","مشروع بدون اسم");t=`${$(i)} <button type="button" class="btn btn-sm btn-outline-primary" data-action="open-project" data-project-id="${N.id}">${$(qe)}</button>`}rt=`
      <div class="res-info-row">
        <span class="label">📁 ${De}</span>
        <span class="value">${t}</span>
      </div>
    `}const H=[];H.push(j("👤",Fe,f?.customerName||e.customerName||Ve)),H.push(j("📞",Te,f?.phone||"—")),H.push(j("🗓️",Ie,Wt)),H.push(j("🗓️",Ee,Jt)),H.push(j("📦",Me,Pt)),H.push(j("⏱️",re,ae)),H.push(j("📝",Re,Je)),rt&&H.push(rt);const ra=H.join(""),la=D.length?D.map(t=>{const i=t.items[0]||{},l=wa(i)||t.image,m=l?`<img src="${l}" alt="${de}" class="reservation-item-thumb">`:'<div class="reservation-item-thumb reservation-item-thumb--placeholder" aria-hidden="true">🎥</div>';let h=[];if(Array.isArray(t.packageItems)&&t.packageItems.length)h=[...t.packageItems];else{const c=[];t.items.forEach(p=>{Array.isArray(p?.packageItems)&&p.packageItems.length&&c.push(...p.packageItems)}),h=c}if(Array.isArray(h)&&h.length>1){const c=new Set;h=h.filter(p=>{const o=p?.normalizedBarcode&&String(p.normalizedBarcode).toLowerCase()||p?.barcode&&String(p.barcode).toLowerCase()||(p?.equipmentId!=null?`id:${p.equipmentId}`:null);return o?c.has(o)?!1:(c.add(o),!0):!0})}const y=st(t)||t.items.some(c=>st(c))||h.length>0,A=(c,{fallback:p=1,max:o=1e3}={})=>{const x=Ke(c);return Number.isFinite(x)&&x>0?Math.min(o,x):p};let v;if(y){const c=A(i?.qty??i?.quantity??i?.count,{fallback:NaN,max:999});Number.isFinite(c)&&c>0?v=c:v=A(t.quantity??t.count??1,{fallback:1,max:999})}else v=A(t.quantity??t.count??i?.qty??i?.quantity??i?.count??0,{fallback:1,max:9999});const q=n(String(v)),r=(c,{preferPositive:p=!1}={})=>{let o=Number.NaN;for(const x of c){const I=C(x);if(Number.isFinite(I)){if(p&&I>0)return I;Number.isFinite(o)||(o=I)}}return o};let s,d;if(y){const c=[i?.price,i?.unit_price,i?.unitPrice,t.unitPrice];if(s=r(c,{preferPositive:!0}),!Number.isFinite(s)||s<0){const o=C(t.totalPrice??i?.total??i?.total_price);Number.isFinite(o)&&v>0&&(s=o/v)}Number.isFinite(s)||(s=0);const p=[i?.total,i?.total_price,t.totalPrice];if(d=r(p),!Number.isFinite(d))d=s*v;else{const o=s*v;Number.isFinite(o)&&o>0&&Math.abs(d-o)>o*.25&&(d=o)}}else{const c=[i?.price,i?.unit_price,i?.unitPrice,t.unitPrice];if(s=r(c,{preferPositive:!0}),!Number.isFinite(s)||s<0){const p=C(t.totalPrice??i?.total??i?.total_price);Number.isFinite(p)&&v>0&&(s=p/v)}Number.isFinite(s)||(s=0),d=C(t.totalPrice??i?.total??i?.total_price),Number.isFinite(d)||(d=s*v)}s=S(s),d=S(d);const b=`${n(s.toFixed(2))} ${_}`,P=`${n(d.toFixed(2))} ${_}`,u=t.barcodes.map(c=>n(String(c||""))).filter(Boolean),L=u.length?`<details class="reservation-item-barcodes">
              <summary>${a("reservations.equipment.barcodes.summary","عرض الباركودات")}</summary>
              <ul class="reservation-barcode-list">
                ${u.map(c=>`<li>${c}</li>`).join("")}
              </ul>
            </details>`:"";let w="";if(h.length){const c=new Map,p=o=>1;if(h.forEach(o=>{if(!o)return;const x=G(o.barcode||o.normalizedBarcode||o.desc||Math.random());if(!x)return;const I=c.get(x),U=p();if(I){I.qty=U,I.total=U;return}c.set(x,{desc:o.desc||o.barcode||a("reservations.create.packages.unnamedItem","عنصر بدون اسم"),qty:Math.max(1,Math.min(U,99)),total:Math.max(1,Math.min(U,99)),barcode:o.barcode??o.normalizedBarcode??""})}),c.size){const o=Array.from(c.values()).map(x=>{const I=n(String(x.qty>0?Math.min(x.qty,99):1)),U=$(x.desc||""),pa=x.barcode?` <span class="reservation-package-items__barcode">(${$(n(String(x.barcode)))})</span>`:"";return`<li>${U}${pa} × ${I}</li>`}).join("");w=`
  ...
  `,Ft=K.map((t,i)=>{const l=n(String(i+1));let m=t.positionLabel??t.position_name??t.position_label??t.position_title??t.role??t.position??null;if((!m||m.trim()==="")&&(m=t.positionLabelAr??t.position_label_ar??t.position_title_ar??t.positionLabelEn??t.position_label_en??t.position_name_ar??t.position_title_en??t.position_name_en??null),!m||m.trim()==="")try{const b=typeof Q=="function"?Q():[],P=t.positionId?b.find(w=>String(w.id)===String(t.positionId)):null,u=!P&&t.positionKey?b.find(w=>String(w.name).toLowerCase()===String(t.positionKey).toLowerCase()):null,L=P||u||null;L&&(m=L.labelAr||L.labelEn||L.name||m)}catch{}const h=ct(m)||a("reservations.crew.positionFallback","منصب بدون اسم"),y=ct(t.positionLabelAlt??t.position_label_alt??t.positionLabelEn??t.position_label_en??t.positionLabelAr??t.position_label_ar??t.position_name_en??t.position_name_ar??""),A=ct(t.technicianName)||a("technicians.picker.noTechnicianOption","— بدون تعيين —"),v=t.technicianPhone||me,q=S(C(t.positionCost??t.position_cost??t.cost??t.daily_wage??t.dailyWage??t.internal_cost??0));let r=S(C(t.positionClientPrice??t.position_client_price??t.client_price??t.customer_price??t.position_price??t.clientPrice??t.daily_total??t.dailyTotal??t.total??0));if(!Number.isFinite(r)||r<=0)try{const b=Q?Q():[],P=t.positionId?b.find(w=>String(w.id)===String(t.positionId)):null,u=!P&&t.positionKey?b.find(w=>String(w.name).toLowerCase()===String(t.positionKey).toLowerCase()):null,L=P||u||null;L&&Number.isFinite(Number(L.clientPrice))&&(r=S(Number(L.clientPrice)))}catch{}const s=`${n(r.toFixed(2))} ${_}`,d=q>0?`${n(q.toFixed(2))} ${_}`:null;return`
    ...
    `}).join(""),da=Array.isArray(K)&&K.length>4,ua=K.length?da?`
        ...
        `:`<div class="reservation-technicians-grid">${Ft}</div>`:`<ul class="reservation-modal-technicians"><li>${pe}</li></ul>`;return`
    <div class="reservation-modal">
      ...
      <div class="reservation-modal-actions">
        ...
        ${Rt?`<button type="button" class="modal-action-btn modal-action-btn--danger" id="reservation-details-delete-btn" data-index="${B}">${Ce}</button>`:""}
      </div>
    </div>
