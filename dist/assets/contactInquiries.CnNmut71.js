import{a as C,c as B,i as N,g as M,l as F,t as i,b as I,A as T,n as x,j as q}from"./auth.Ds2WBl9G.js";import"./dashboard.CrdD3q6G.js";import{i as D}from"./dashboardShell.DF2kUed9.js";C();B();D();N();const e={};let v=null,b=[],u=0,w=null,h=!1;const f={new:{badge:"badge-neutral",fallbackAr:"جديدة",fallbackEn:"New",key:"contactInquiries.status.new"},in_progress:{badge:"badge-warning",fallbackAr:"قيد المتابعة",fallbackEn:"In Progress",key:"contactInquiries.status.inProgress"},contacted:{badge:"badge-info",fallbackAr:"تم التواصل",fallbackEn:"Contacted",key:"contactInquiries.status.contacted"},won:{badge:"badge-success",fallbackAr:"تم التحويل",fallbackEn:"Won",key:"contactInquiries.status.won"},lost:{badge:"badge-error",fallbackAr:"مفقودة",fallbackEn:"Lost",key:"contactInquiries.status.lost"},closed:{badge:"badge-outline",fallbackAr:"مغلقة",fallbackEn:"Closed",key:"contactInquiries.status.closed"}};function r(a){return document.querySelector(a)}function o(a){return String(a??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function H(a){return o(a).replace(/\n/g,"<br>")}function p(a){const t=String(a||"").trim();if(!t)return"—";const n=t.includes("T")?t:t.replace(" ","T"),s=new Date(n);if(Number.isNaN(s.getTime()))return t;try{return new Intl.DateTimeFormat(document.documentElement.lang||"ar",{year:"numeric",month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}).format(s)}catch{return t}}function j(a){const t=String(a||"").trim();return t?t.slice(0,16).replace(" ","T"):""}function U(a){const t=String(a||"").trim();if(!t)return!1;const n=new Date(t.includes("T")?t:t.replace(" ","T"));if(Number.isNaN(n.getTime()))return!1;const s=new Date;return n.getFullYear()===s.getFullYear()&&n.getMonth()===s.getMonth()&&n.getDate()===s.getDate()}function k(a){const t=f[String(a||"").trim()]||f.new;return i(t.key,document.documentElement.lang==="en"?t.fallbackEn:t.fallbackAr)}function P(a){const t=String(a||"new").trim();return`<span class="badge ${(f[t]||f.new).badge}">${o(k(t))}</span>`}function O(){e.logoutBtn=r("#logout-btn"),e.refreshBtn=r("#contact-inquiries-refresh"),e.statusFilter=r("#contact-status-filter"),e.searchFilter=r("#contact-search-filter"),e.tableBody=r("#contact-inquiries-body"),e.detailContent=r("#contact-inquiry-detail-content"),e.workflow=r("#contact-inquiry-workflow"),e.activityList=r("#contact-inquiry-activity-list"),e.detailStatusBadge=r("#contact-detail-status-badge"),e.statNew=r("#contact-stat-new"),e.statProgress=r("#contact-stat-progress"),e.statFollowUpToday=r("#contact-stat-followup-today"),e.statClosed=r("#contact-stat-closed")}function R(){const a=String(v?.role||"").toLowerCase(),t=a==="admin",n=t||a==="manager";document.querySelectorAll("[data-admin-card]").forEach(s=>{s.classList.toggle("hidden",!t)}),document.querySelectorAll("[data-manager-card]").forEach(s=>{s.classList.toggle("hidden",!n)})}function z(){document.body.classList.remove("auth-pending")}function L(a){const t=Array.isArray(a)?a:[],n=s=>t.filter(d=>String(d?.status||"")===s).length;if(e.statNew&&(e.statNew.textContent=x(String(n("new")))),e.statProgress){const s=n("in_progress")+n("contacted");e.statProgress.textContent=x(String(s))}if(e.statFollowUpToday){const s=t.filter(d=>U(d?.follow_up_at)).length;e.statFollowUpToday.textContent=x(String(s))}if(e.statClosed){const s=n("closed")+n("won")+n("lost");e.statClosed.textContent=x(String(s))}}function _(a){if(e.tableBody){if(!Array.isArray(a)||a.length===0){e.tableBody.innerHTML=`
  ...
  `;try{const t=new URLSearchParams,n=String(e.statusFilter?.value||"").trim(),s=String(e.searchFilter?.value||"").trim();n&&t.set("status",n),s&&t.set("search",s),t.set("limit","100");const d=await I(`/contact/admin.php?${t.toString()}`);if(b=Array.isArray(d?.data)?d.data:[],L(b),_(b),!a){u=0,m();return}if(u>0&&b.some(g=>Number(g.id)===Number(u))){await S(u,{silentOnError:!0});return}b.length>0?await S(Number(b[0].id),{silentOnError:!0}):(u=0,m())}catch(t){console.error("Failed to load contact inquiries",t),L([]),e.tableBody.innerHTML=`
    ...
    `,m()}}}async function S(a,{silentOnError:t=!1}={}){const n=Number(a||0);if(n<=0){m();return}try{const s=await I(`/contact/admin.php?id=${encodeURIComponent(n)}`);u=n,_(b),A(s?.data??null)}catch(s){console.error("Failed to load inquiry details",s),t||q(s instanceof T?s.message:i("contactInquiries.details.error","تعذر تحميل تفاصيل الرسالة."),"error")}}async function $({markContacted:a=!1,assignToMe:t=!1}={}){if(u<=0||h)return;const n=r("#contact-detail-status"),s=r("#contact-detail-follow-up"),d=r("#contact-detail-notes"),c=r("#contact-save-btn"),g={id:u,status:n?.value||"new",follow_up_at:s?.value||"",internal_notes:d?.value||""};a&&(g.mark_contacted=!0),t&&(g.assign_to_me=!0),h=!0,[c,r("#contact-mark-contacted-btn"),r("#contact-assign-btn")].forEach(l=>{l&&(l.disabled=!0)});try{const l=await I("/contact/admin.php",{method:"PATCH",body:g});A(l?.data??null),await y(),q(i("contactInquiries.workflow.saved","تم حفظ التحديثات بنجاح."),"success")}catch(l){console.error("Failed to save inquiry changes",l),q(l instanceof T?l.message:i("contactInquiries.workflow.saveError","تعذر حفظ التحديثات."),"error")}finally{h=!1,[c,r("#contact-mark-contacted-btn"),r("#contact-assign-btn")].forEach(l=>{l&&(l.disabled=!1)})}}function V(){e.logoutBtn&&!e.logoutBtn.dataset.listenerAttached&&(e.logoutBtn.addEventListener("click",()=>F()),e.logoutBtn.dataset.listenerAttached="true"),e.refreshBtn?.addEventListener("click",()=>{y({preserveSelection:!0})}),e.statusFilter?.addEventListener("change",()=>{y({preserveSelection:!1})}),e.searchFilter?.addEventListener("input",()=>{w&&clearTimeout(w),w=setTimeout(()=>{y({preserveSelection:!1})},300)})}async function E(){O(),V(),m();try{v=await M({refresh:!0})}catch{v=null}if(!v){window.location.href="login.html";return}const a=String(v?.role||"").toLowerCase();if(a!=="admin"&&a!=="manager"){window.location.href="home.html";return}R(),z(),await y({preserveSelection:!0})}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",()=>{E().catch(a=>{console.error("Failed to bootstrap contact inquiries page",a)})},{once:!0}):E().catch(a=>{console.error("Failed to bootstrap contact inquiries page",a)});
