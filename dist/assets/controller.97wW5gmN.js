        ...
        ${T.length?`<div class="info-plain">${T.join("")}</div>`:q}
          ...
          <div class="totals-block">
            ...
            <table class="quote-table">
              ...
              <tbody>${ae.length?ae.map((S,J)=>`<tr>${O.map(w=>`<td><div class="quote-cell">${w.render(S,J)}</div></td>`).join("")}</tr>`).join(""):`<tr><td colspan="${Math.max(O.length,1)}" class="empty">${E(o("projects.details.equipment.empty","لا توجد معدات مرتبطة حالياً."))}</td></tr>`}
              </tbody>
            </table>
