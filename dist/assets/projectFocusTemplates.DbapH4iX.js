import{n as m,t,p as Ue}from"./auth.B4XUmSYg.js";import{c as We,e as s,d as Ce,t as Je,f as y}from"./state.DnWIUPO4.js";import{e as we,c as Xe,d as Ye,g as Pe,a as Ke,i as Ge,D as xe,u as Qe}from"./reservationsService.DZ-N01gk.js";import{f as Te}from"./view.Dbljb8vp.js";const Q=.15,Ze=2,Ne={upcoming:"Upcoming",ongoing:"In Progress",completed:"Completed"},et={upcoming:"timeline-status-badge timeline-status-badge--upcoming",ongoing:"timeline-status-badge timeline-status-badge--ongoing",completed:"timeline-status-badge timeline-status-badge--completed"};function be(e){if(!e)return null;const n=[e?.id,e?.reservationId,e?.reservation_id,e?.reservationID].find(i=>i!=null&&i!=="");return n!=null?String(n):null}function tt(e,a){if(!e)return{dateHtml:"—",timeText:""};const n=Te(e),i=a?Te(a):"",o=u=>{const b=String(u).split(" ").filter(Boolean),v=b.shift()||"",T=b.join(" ");return{date:v,time:T}},c=o(n),l=i?o(i):{date:"",time:""};let r="",d="";return l.date&&c.date===l.date?(r=`<div class="date-range"><div class="date-line">${c.date}</div></div>`,d=`من ${c.time||"—:—"} إلى ${l.time||"—:—"}`):(r=`<div class="date-range"><div class="date-line">${c.date}</div>`+(l.date?`<div class="date-line">${l.date}</div>`:"")+"</div>",d=`من ${c.time||"—:—"} إلى ${l.time||"—:—"}`),{dateHtml:r,timeText:d}}function ve(e=[]){const a=Pe();return e.map(n=>{const i=n?.reservation??n;if(!i||typeof i!="object")return null;let o=Number.isInteger(n?.index)&&n.index>=0?n.index:Number.isInteger(n?.reservationIndex)&&n.reservationIndex>=0?n.reservationIndex:null;if(!Number.isInteger(o)||o<0){const c=be(i);c&&(o=a.findIndex(l=>{const r=be(l);return r&&r===c}))}return{reservation:i,index:Number.isInteger(o)&&o>=0?o:-1}}).filter(Boolean)}function he(e){return e?e.id!=null?String(e.id):e.projectId!=null?String(e.projectId):e.project_id!=null?String(e.project_id):null:null}function vt(e,{customer:a=null,techniciansMap:n=new Map,reservations:i=[]}={}){const c=ve(i).map(({reservation:$})=>$),l=c.length,r=he(e),d=r?s(r):"",u=Ce(e),b=t(`projects.status.${u}`,Ne[u]),v=et[u]||"bg-secondary",T=ye(e)||{},_=Number(T.subtotal||0),x=(c||[]).reduce(($,h)=>$+oe(h),0),j=T.applyTax?Number(((_+x)*Q).toFixed(2)):0,f=Number((_+x+j).toFixed(2)),N=Array.isArray(e?.paymentHistory)?e.paymentHistory:Array.isArray(e?.payments)?e.payments:[],F=Xe({totalAmount:f,paidAmount:N.length?0:e.paidAmount,paidPercent:N.length?0:e.paidPercent,history:N}),S=Ye({manualStatus:null,paidAmount:F.paidAmount,paidPercent:F.paidPercent,totalAmount:f}),L=t(`projects.paymentStatus.${S}`,S==="paid"?"Paid":S==="partial"?"Partially Paid":"Unpaid"),A=S==="paid"?"status-paid":S==="partial"?"status-partial":"status-unpaid",E=[S==="paid"?"project-focus-card--paid":"project-focus-card--unpaid"],q=e?.confirmed===!0||e?.confirmed==="true";q&&E.push("project-focus-card--confirmed");const B=e?.projectCode||(r?`PRJ-${m(r)}`:""),I=B?m(String(B).replace(/^#/,"")):"",J=I?`<span class="project-code-badge project-focus-card__code">#${s(I)}</span>`:"",V=ge(e?.type),z="",Z=`<span class="${v}">${s(b)}</span>`,ie=`<span class="reservation-chip ${A} project-focus-card__payment-chip">${s(L)}</span>`,ce=(e?.title||"").trim()||t("projects.fallback.untitled","Untitled project"),ee=(e?.description||"").trim(),le=ee?s(Je(ee,110)):s(t("projects.fallback.noDescription","No description")),g=Array.isArray(e?.technicians)?e.technicians:[],te=g.map($=>n.get(String($))?.name).filter(Boolean);te.length&&at(te);const se=a?.customerName||e?.clientName||"";(e?.clientCompany||a?.companyName||"").trim();const p=c.reduce(($,h)=>{const G=oe(h),je=Array.isArray(h?.items)?h.items:[],Me=je.reduce((ze,Oe)=>ze+(Number(Oe?.qty)||0),0),Be=Array.isArray(h?.technicians)?h.technicians.length:0,$e=Array.isArray(h?.crewAssignments)?h.crewAssignments:[],M=$e.length?$e:Array.isArray(h?.technicians)?h.technicians:[],me=we({items:je,technicianIds:Array.isArray(M)&&!M.length?M:[],crewAssignments:Array.isArray(M)&&M.length&&typeof M[0]=="object"?M:[],discount:h?.discount??0,discountType:h?.discountType||"percent",applyTax:!1,start:h?.start,end:h?.end,companySharePercent:null});return{total:$.total+G,equipment:$.equipment+Number(me.equipmentTotal||0),crew:$.crew+Number(me.crewTotal||0),crewCost:$.crewCost+Number(me.crewCostTotal||0),equipmentCountTotal:($.equipmentCountTotal||0)+Me,crewCountTotal:($.crewCountTotal||0)+Be}},{total:0,equipment:0,crew:0,crewCost:0,equipmentCountTotal:0,crewCountTotal:0}),C=Number(p.total.toFixed(2)),w=p.equipmentCountTotal||0,X=p.crewCountTotal||g.length,de=ye(e),P=Number(e?.servicesClientPrice??0),O=Number((p.equipment+p.crew+P).toFixed(2)),Y=Number.parseFloat(e?.discount??e?.discountValue??0)||0;let R=(e?.discountType==="amount"?"amount":"percent")==="amount"?Y:O*(Y/100);(!Number.isFinite(R)||R<0)&&(R=0),R>O&&(R=O);const K=Math.max(0,O-R),U=e?.companyShareEnabled===!0||e?.companyShareEnabled==="true"||e?.company_share_enabled===!0||e?.company_share_enabled==="true",D=Number.parseFloat(e?.companySharePercent??e?.company_share_percent??e?.companyShare??e?.company_share??0)||0,H=U&&D>0?D:0,ne=Number((K*(H/100)).toFixed(2)),pe=de.applyTax?Number(((K+ne)*Q).toFixed(2)):0,W=Number((K+ne+pe).toFixed(2)),{dateHtml:ue,timeText:k}=tt(e?.start,e?.end),Re=[I?{icon:"🆔",label:t("projectCards.meta.code","رقم المشروع"),value:`#${I}`}:null,se?{icon:"👤",label:t("projectCards.meta.client","العميل"),value:se}:null,V?{icon:"🏷️",label:t("projects.details.type","نوع المشروع"),value:`<span class="project-type-chip project-type-chip--${e.type||"default"}">${s(V)}</span>`}:null,{icon:"📅",label:t("projects.focus.summary.range","الفترة الزمنية"),value:ue},k?{icon:"⏰",label:t("projects.focus.summary.time","الوقت"),value:k}:null].filter(Boolean);t("projectCards.stats.reservationsShort","الحجوزات"),m(String(l)),t("projectCards.stats.equipmentCount","عدد المعدات"),m(String(w)),t("projectCards.stats.crewCount","عدد أفراد الطاقم"),m(String(X)),t("projectCards.stats.reservationValue","إجمالي الحجوزات"),y(C);const Fe=H>0&&de.applyTax?` ${t("projects.details.chips.vatOn","(شامل الضريبة)")}`:"",Ie=`${t("projects.details.summary.finalTotal","المجموع النهائي")}${Fe}`,De=[{icon:"💼",label:t("projectCards.stats.servicesClientPrice","الخدمات الإنتاجية"),value:y(P)},{icon:"💵",label:Ie,value:y(W)}].map(({icon:$,label:h,value:G})=>fe($,h,G)).join(""),ke=[{icon:"🔗",label:t("projectCards.stats.reservationsShort","الحجوزات"),value:m(String(l))},{icon:"📦",label:t("projectCards.stats.equipmentCount","عدد المعدات"),value:m(String(w))},{icon:"😎",label:t("projectCards.stats.crewCount","عدد الطاقم"),value:m(String(X))},{icon:"💵",label:t("projectCards.stats.reservationValue","إجمالي الحجوزات"),value:y(C)}].map(({icon:$,label:h,value:G})=>fe($,h,G)).join(""),Le=[st("projects.focus.summary.project","ملخص المشروع",Re),Se("projects.focus.summary.reservations","الحجوزات المرتبطة",ke),Se("projects.focus.summary.payment","الملخص المالي",De)].filter(Boolean).join(""),Ee=t("projects.focus.confirmed","✅ مشروع مؤكد"),qe=t("projects.focus.pending","⌛ بانتظار التأكيد"),Ve=`<div class="project-focus-card__actions"><span class="reservation-chip ${q?"status-confirmed":"status-pending"} project-focus-card__confirm-indicator">${s(q?Ee:qe)}</span></div>`,He=[J,z,Z,ie].filter(Boolean).join(`
          `);return`
  ...
  `}function it(e){const a=["commercial","coverage","photography","social"],n=a.map(o=>{const c=s(t(`projects.form.types.${o}`,o)),l=String(o)===String(e)?" selected":"";return`<option value="${o}"${l}>${c}</option>`});if(e&&!a.includes(e)){const o=s(ge(e));n.push(`<option value="${s(String(e))}" selected>${o}</option>`)}return`<option value="">${s(t("projects.form.placeholders.type","اختر نوع المشروع"))}</option>${n.join("")}`}function ct(e=[]){if(!Array.isArray(e)||e.length===0)return`<div class="text-muted small" data-empty>${s(t("projects.selected.emptyExpenses","لم يتم تسجيل أي مصروف"))}</div>`;const a=s(t("actions.remove","إزالة"));return e.map(n=>{const i=s(n?.label||""),o=s(y(n?.amount||0)),c=s(String((n?.note??n?.notes)||"")),l=s(String(n?.id||""));return`
      ...
      `}).join("")}function _e(e){if(!e)return{date:"",time:""};let a=e;a.includes(" ")&&(a=a.replace(" ","T"));const[n="",i=""]=a.split("T"),o=i.match(/(\d{1,2}:\d{2})/);return{date:n?n.slice(0,10):"",time:o?o[0]:""}}function jt(e,a){if(!e)return"";const n=a&&/\d{1,2}:\d{2}/.test(a)?a:"00:00",[i="00",o="00"]=n.split(":"),c=i.padStart(2,"0"),l=o.padStart(2,"0");return`${e}T${c}:${l}`}function lt(e){if(!e)return null;const a=e.projectId??e.project_id??e.projectID??null;return a!=null?String(a):null}async function $t(e,a){if(!e)return;const i=Pe().filter(r=>{const d=lt(r);return d&&d===String(e)});if(!i.length)return;const o=a==="paid",c=o?"paid":"unpaid";let l=!1;for(const r of i){const d=r?.id??r?.reservationId??r?.reservation_id;if(!d)continue;const u=r?.paid===!0||r?.paid==="paid",b=r?.paidStatus??r?.paymentStatus??(u?"paid":"unpaid");if(!(u===o&&b===c))try{await Qe(d,{paid_status:c,paid:o}),l=!0}catch(v){console.error("❌ [projectFocusTemplates] Failed to sync reservation payment status",v)}}l&&document.dispatchEvent(new CustomEvent("reservations:changed"))}export{Q as P,ht as a,vt as b,gt as c,jt as d,ct as e,oe as f,he as g,lt as h,ye as r,$t as s};
