...
function updateCustomerProjects() {
  ...
  const { projects = [], technicians = [] } = loadData();
  ...
  container.innerHTML = filtered.map((project) => buildProjectCard(project, techniciansMap)).join('');
}

function buildProjectCard(project, techniciansMap) {
